...
//      	rowdata = new Object[8];
	...
	private void addPersonalAccount(java.awt.event.ActionEvent event) {
		...
		if (newaccount) {
			...
			BankCustomer customer = bank.customers.stream().filter(cust -> cust.getName().equals(customData.get("name"))
					&& cust.getClass().getSimpleName().equals("Person")).findFirst().orElse(null);
			...
			bank.accountsManager.addAccount(acc);
	...
	private void addCompanyAccount(java.awt.event.ActionEvent event) {
		...
		if (newaccount) {
			...
			currentFactoryAccount = BankCustomerAccountsBuilder.getFactoryAccount(factoryType);
			BankCustomer customer = bank.customers.stream().filter(cust -> cust.getName().equals(customData.get("name"))
					&& cust.getClass().getSimpleName().equals("Company")).findFirst().orElse(null);
	...
	private void deposit(java.awt.event.ActionEvent event) {
		...
		if (selection >= 0) {
			...
			BankAccount found = bank.accountsManager.getListAccounts().stream()
	...
	private void addInterest(java.awt.event.ActionEvent event) {
		...
		JOptionPane.showMessageDialog(JButton_Addinterest, "Added interest to all accounts",
